...
document.addEventListener("DOMContentLoaded", function () {
    ...
    saveQuizBtn.addEventListener("click", function () {
        ...
        const answerBoxes = document.querySelectorAll(".answer"); // Each answer box

        let answers = [];

        answerBoxes.forEach(box => {
            const input = box.querySelector(".answer-input");
            const check = box.querySelector(".correct-check");

            const answerText = input?.value.trim();
            const isCorrect = check?.classList.contains("selected");
        ...
        quizData.push({ question: questionInput, answers });
        localStorage.setItem("savedQuizzes", JSON.stringify(quizData));

        console.log("Saved quiz:", quizData);
